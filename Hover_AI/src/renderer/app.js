...
captureButton.addEventListener("click",async () => {
    ...
    const question = "How to open a new terminal in VS Code ?"
    const result = await window.electronAPI.analyzeImage(
        ...
        You will also receive OCR data after this prompt.

        Each OCR entry has the format:

        Text (x, y, width, height)

        Where:
        - x and y are the top-left pixel coordinates.
        - width and height are the OCR bounding box dimensions.

        HOW TO USE OCR:

        - Use the screenshot to understand the UI layout, icons, windows, and overall context.
        - Use OCR whenever you need to locate visible text because OCR coordinates are more accurate than visual estimation.
        - If the requested element contains text that appears in the OCR data, use the OCR coordinates to determine its location.
        - If the target has no visible text (for example an icon), use the screenshot only.
        - If both screenshot and OCR disagree slightly, prefer the OCR position for text-based elements.
        - Ignore OCR entries that are clearly unrelated to the user's request.
        - OCR data is provided after this prompt under the heading "OCR Data:".
        `,
        words
    )
